...
class DragonMethods {
    ...
    static <E extends Dragon> List<E> getAllChosenDragons(List<Dragon> dragons, Class<E> dragonClass) {
        List<E> list = new ArrayList<>();
        for (Dragon dragon : dragons) {
            if (dragon.getClass().isAssignableFrom(dragonClass)) {
                list.add((E) dragon);
            }
        }
        return list;
    ...
    static List<Hydra> getAllHydrasWithMoreThanAverageHeadsNum(List<Dragon> dragons) {
        List<Hydra> hydras = getAllChosenDragons(dragons, Hydra.class);
    ...
    static List<DeathDragon> getAllPowerfulDeathDragons(List<Dragon> dragons, int power) {
        List<DeathDragon> deathDragons = getAllChosenDragons(dragons, DeathDragon.class);
    ...
    static List<WaterDragon> getAllWaterDragonsInFirstQuater(List<Dragon> dragons) {
        List<WaterDragon> waterDragons = getAllChosenDragons(dragons, WaterDragon.class);
    ...
    /**3.5)методы, которые подсчитывают время сортировки масива*/
