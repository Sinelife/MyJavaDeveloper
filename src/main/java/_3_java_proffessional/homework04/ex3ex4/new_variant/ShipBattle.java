...
class ShipBattle {
    ...
    private void initializePlayerList() {
        ...
        if (compPlayerNum == 1) {
            players[0] = new Player(true, 1);
            players[1] = new Player(false, 2);
        } else if (compPlayerNum == 2) {
            players[0] = new Player(false, 1);
            players[1] = new Player(true, 2);
        } else {
            players[0] = new Player(false, 1);
            players[1] = new Player(false, 2);
        }
     ...
     */
    private void battle() {
        CopyService.addStartBattleMessageToFile();
        System.out.println(BATTLE_MESSAGE);
        Player player = players[0];
        while (true) {
            CopyService.addRoundInfoToFile(roundNumber);
            roundNumber++;
    ...
    private void outputWinningMessage(Player player) {
        ...
        result += "!!!(" + "Осталось корабей: " + player.getAliveShipNum() + ")";
        System.out.println(result);
        CopyService.addWinningMessageToFile(result);
    }
