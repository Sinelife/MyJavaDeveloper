...
import static _3_java_proffessional.homework04.ex3ex4.new_variant.Constants.*;


class Player {
    private static final String FIELD_PATH_FILE = "C:\\Users\\Ярослав\\JavaDeveloper\\src\\main\\other\\java_proffessional\\homework04\\ex3ex4\\new_variant\\fields\\field5_427_441.txt";
    ...
    Player(boolean isComputer, int turnNumber) {
    ...
    private static void inputBoardSize() {
        System.out.println(INPUT_BOARD_SIZE);
    ...
    private static void inputShipNum() {
        System.out.println(INPUT_SHIP_NUMBER);
    ...
    /**
     * Метод, для разстановки флота(вручную либо с прчтением из файла)
     */
    void placeFleet() {
        boolean readFieldFromFile = toReadFieldFromFileOrNot();
        if (!readFieldFromFile) {
            placeFleetByInput();
        } else {
            CopyService.setBoard(this, new File(FIELD_PATH_FILE));
        }
    }

    /**
     * Метод, для выбора считывать поле игрока с файла или нет
     */
    private boolean toReadFieldFromFileOrNot() {
        if(!isComputer) {
            System.out.println("Введите 0 - если хотите разместить корабли вручную, 1 - если хотите считать с файла.");
            int i = personInput(0, 1);
            return i != 0;
        }
        return false;
    }
    ...
    /**
     * Метод, для разстановки флота вручную
     */
    private void placeFleetByInput() {
        CopyService.addFleetPlacingMessageToFile(this);
        int counter = 1;
        while (counter <= shipNum) {
            outputBoard();
            int x = inputCoordinate(INPUT_VERTICAL);
            int y = inputCoordinate(INPUT_HORIZONTAL);
            if (boardBoxIsEmpty(x, y) && !isNearShip(x, y)) {
                placeShip(x, y);
                counter++;
            } else {
                outputMessage(REPEAT_PLACE_MESSAGE);
            }
        }
        CopyService.addSpaceBetweenRows();
        CopyService.writeFieldToFile(this);
    }
    ...
    private void placeShip(int startX, int startY) {
        ...
        while (true) {
            outputMessage(SAME_SHIP_PLACE_CONTINUE_MESSAGE);
            if (isEndOfThisShip()) {
                break;
            }
            outputBoard();
            ...
            if (boardBoxIsEmpty(x, y) && isNearShip(x, y)) {
                ...
                outputMessage(WRONG_SHIP_PART_PLACE_MESSAGE);
            }
        }
        ships.add(ship);
        CopyService.addShipCoordinatesToFile(ship);
    }
    ...
    void shot(Player player) {
        while (true) {
            ...
            if (shootIsNotRepeatInSamePlace(x, y, player)) {
                ...
                outputMessage(REPEAT_SHOT_MESSAGE);
            }
        }
    }
    ...
    private void changeStatusOfBoxAfterShot(Player player, int x, int y) {
        CopyService.addShotMessageToFile(this, x, y);
        if (shotWasLuck(player, x, y)) {
            brokeShipPart(player, x, y);
        } else {
            CopyService.addShotMissMessageToFile();
        }
    ...
    String outputFleetStatus() {
        ...
        for (int i = 1, counter = 0; counter < getAliveShipNum(); i++) {
            ...
            if (counter != getAliveShipNum() && num != 0) {
                res += ", ";
            }
        }
        return res + ").";
